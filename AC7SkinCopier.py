...
def replace_patterns(data, old_xxxx, new_xxxx, old_yy, new_yy):
    """
    Safely replaces PlaneID and SkinID patterns throughout the JSON structure.
    Prevents overlapping and partial replacements such as 'su37uu' when PlaneID = 'su37u'.
    """

    # Escape any regex-sensitive characters (just in case)
    old_xxxx_esc = re.escape(old_xxxx)
    new_xxxx_esc = re.escape(new_xxxx)
    old_yy_esc = re.escape(old_yy)
    new_yy_esc = re.escape(new_yy)

    replacements = [
        # --- Specific patterns first (with _sXX) ---
        (rf"(?<!\w)AcePlayerPawn_{old_xxxx_esc}_s{old_yy_esc}(?!\w)", f"AcePlayerPawn_{new_xxxx}_s{new_yy}"),
        (rf"(?<!\w)AcePlayerPawn_{old_xxxx_esc}_s{old_yy_esc}_C(?!\w)", f"AcePlayerPawn_{new_xxxx}_s{new_yy}_C"),
        (rf"(?<!\w)Default__AcePlayerPawn_{old_xxxx_esc}_s{old_yy_esc}_C(?!\w)", f"Default__AcePlayerPawn_{new_xxxx}_s{new_yy}_C"),
        (rf"/Game/Blueprint/Player/Pawn/Skin/AcePlayerPawn_{old_xxxx_esc}_s{old_yy_esc}\b", f"/Game/Blueprint/Player/Pawn/Skin/AcePlayerPawn_{new_xxxx}_s{new_yy}"),
        (rf"/Game/Blueprint/Player/Pawn/AcePlayerPawn_{old_xxxx_esc}_s{old_yy_esc}\b", f"/Game/Blueprint/Player/Pawn/AcePlayerPawn_{new_xxxx}_s{new_yy}"),

        # --- Vehicle assets (with PlaneID/SkinID folders) ---
        (rf"/Game/Vehicles/Aircraft/{old_xxxx_esc}/{old_yy_esc}/{old_xxxx_esc}_{old_yy_esc}_CP_Inst\b", f"/Game/Vehicles/Aircraft/{new_xxxx}/{new_yy}/{new_xxxx}_{new_yy}_CP_Inst"),
        (rf"/Game/Vehicles/Aircraft/{old_xxxx_esc}/{old_yy_esc}/{old_xxxx_esc}_{old_yy_esc}_Decal_Inst\b", f"/Game/Vehicles/Aircraft/{new_xxxx}/{new_yy}/{new_xxxx}_{new_yy}_Decal_Inst"),
        (rf"/Game/Vehicles/Aircraft/{old_xxxx_esc}/{old_yy_esc}/{old_xxxx_esc}_{old_yy_esc}_Inst\b", f"/Game/Vehicles/Aircraft/{new_xxxx}/{new_yy}/{new_xxxx}_{new_yy}_Inst"),

        # --- Short asset names ---
        (rf"(?<!\w){old_xxxx_esc}_{old_yy_esc}_CP_Inst(?!\w)", f"{new_xxxx}_{new_yy}_CP_Inst"),
        (rf"(?<!\w){old_xxxx_esc}_{old_yy_esc}_Decal_Inst(?!\w)", f"{new_xxxx}_{new_yy}_Decal_Inst"),
        (rf"(?<!\w){old_xxxx_esc}_{old_yy_esc}_Inst(?!\w)", f"{new_xxxx}_{new_yy}_Inst"),

        # --- General Pawn / class references (done last) ---
        (rf"/Game/Blueprint/Player/Pawn/AcePlayerPawn_{old_xxxx_esc}\b", f"/Game/Blueprint/Player/Pawn/AcePlayerPawn_{new_xxxx}"),
        (rf"(?<!\w)AcePlayerPawn_{old_xxxx_esc}(?!\w)", f"AcePlayerPawn_{new_xxxx}"),
        (rf"(?<!\w)AcePlayerPawn_{old_xxxx_esc}_C(?!\w)", f"AcePlayerPawn_{new_xxxx}_C"),
        (rf"(?<!\w)Default__AcePlayerPawn_{old_xxxx_esc}_C(?!\w)", f"Default__AcePlayerPawn_{new_xxxx}_C"),
    ]
...
def main():
    root = tk.Tk()
    root.title("AC7SkinCopier JSON/UAsset Generator")
